...
reactor_test() ->
    ...
    meck:new(myreactor, [non_strict]),
    meck:expect(myreactor, initialize, fun([A, B]) ->
        {continue, add, #{a => A, b => B}}
    end),
    meck:expect(myreactor, handle_call, fun
    ...
    meck:expect(myreactor, handle_cast, fun
    ...
    meck:expect(myreactor, handle_message, fun
        (Operation, {Pid, result}, #{a := A, b := B} = Data) ->
            Pid ! {self(), compute_result(Operation, A, B)},
            {continue, Operation, Data};
        (_Operation, switch, _Data) ->
    ...
    Root = self(),
    meck:expect(myreactor, handle_transition, fun(From, To, Data) ->

        Root ! {transition, From, To, Data},
        {continue, Data}
    end),

    {ok, Pid} = reactor:start(link, myreactor, [42, 69]),

    {reply, 111} = reactor:call(Pid, result),
    reactor:cast(Pid, {self(), result}),
    ok = receive
        {Pid, 111} -> ok
    end,
    Pid ! {self(), result},
    ...
    {reply, ok} = reactor:call(Pid, switch),
    ...
    {reply, 27} = reactor:call(Pid, result),
    reactor:cast(Pid, {self(), result}),
    ok = receive
        {Pid, 27} -> ok
    end,
    ...
    ok = reactor:cast(Pid, switch),
    ok = receive
        {transition, subtract, multiply, #{a := 10, b := 100}} ->
            ok
    end,

    {reply, 1000} = reactor:call(Pid, result),
    reactor:cast(Pid, {self(), result}),
    ok = receive
        {Pid, 1000} -> ok
    end,
    ...
    % false = is_process_alive(Pid, 50),

    meck:unload(myreactor),
